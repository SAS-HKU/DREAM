...
CARFIG_DIR = os.path.join(dirpath, "carfigs")
FILE_SAVE_DIR = os.path.join(dirpath, "file_save")
...
car_image = plt.imread(os.path.join(CARFIG_DIR, "black&white.png"))
...
car_image_surround_1 = plt.imread(os.path.join(CARFIG_DIR, "yellow.png"))
car_image_surround_2 = plt.imread(os.path.join(CARFIG_DIR, "red.png"))
...
def metrics_save(S_obs_record,initial_params,progress_20,progress_40,TTC_record,vel,vys,ys,acc,steer_record,lane_state_record,path_record,C_label_record,initial_vds,iter_num,round_=None,comparison=None):
    ...
    if comparison is not None:
        output_dir = os.path.join(dirpath, comparison, "file_save")
    else:
        output_dir = FILE_SAVE_DIR
    os.makedirs(output_dir, exist_ok=True)
    with open(os.path.join(output_dir, time_now), 'wb') as file:
