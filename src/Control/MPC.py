...
class LMPC:
    def __init__(self,NX,NU,T,TARGET_SPEED,MAX_ACCEL,MIN_ACCEL,
        ...
        # DRIFT risk field integration (set externally by PRIDEAMController)
        # risk_cost_vector: array of risk values along MPC horizon [T+1]
        # risk_weight: scalar weight for risk penalty in cost function
...
##################################### functions for iMPC-DOHCBF ###########################################
    ...
    def iMPC_solve_OneStep(self, path_d,path_dindex, x0, x0_g, oa, od, GPR_vy, GPR_w, C_label, C_label_additive,C_label_virtual,
        ...
        for t in range(self.T):
            ...
            # DRIFT risk penalty: penalize speed in high-risk regions
            # Adds w_risk * 0.1 * R(t) * vx(t)^2 to the cost function.
            # The 0.1 internal scale calibrates risk_weight=0.5 to yield
            # ~5 m/s speed reduction at risk=1.0 (given Q[0,0] ~ 0.2).
            if self.risk_cost_vector is not None and self.risk_weight > 0:
                _ri = min(t, len(self.risk_cost_vector) - 1)
                _r_t = float(self.risk_cost_vector[_ri])
                if _r_t > 0.05:
                    cost += self.risk_weight * 0.1 * _r_t * cvxpy.square(x[0, t])
        ...
        # Terminal DRIFT risk cost (same formulation as per-step penalty)
        if self.risk_cost_vector is not None and self.risk_weight > 0:
            _ri = min(self.T, len(self.risk_cost_vector) - 1)
            _r_T = float(self.risk_cost_vector[_ri])
            if _r_T > 0.05:
                cost += self.risk_weight * 0.1 * _r_T * cvxpy.square(x[0, self.T])
