...
class LeaderFollower_Uitl:
    ...
    def get_all_constraint(self,C_label,path_now,path_d,ego_group,path_ego,X0,X0_g,target_group,vehicle_left,vehicle_centre,vehicle_right,path_dindex,C_label_virtual):
        ...
        if C_label_additive == "No Probe":
            ...
            _sf_ego_row = prediction_sf_ego[0,:] if prediction_sf_ego is not None else None
            if prediction_sl_ego is None:
                return prediction_sl_ego, _sf_ego_row, second_param_l, third_param_l, surround_constraints
            else:
                return prediction_sl_ego[0,:], _sf_ego_row, second_param_l, third_param_l, surround_constraints
            ...
            if prediction_sl_target is None:
                return prediction_ahead, prediction_rear, _sf_ego_row, prediction_sl_target, second_param_tl, third_param_tl, surround_constraints
            else:
                return prediction_ahead, prediction_rear, _sf_ego_row, prediction_sl_target[0,:], second_param_tl, third_param_tl, surround_constraints
    ...
    def get_remap_vehicles(self,x0_g_l,prediction_vl_ego,path_dindex,path_d):
        if x0_g_l[0] is not None and prediction_vl_ego is not None:
